...
const DOSE_SYSTEM_PROMPT = `You are a clinical dosing support assistant used by pharmacy staff. You are NOT a replacement for a licensed pharmacist or physician — every output must be clearly labeled as a computed suggestion.

You will receive structured patient data: age, sex, height, weight, BMI, calculated BSA, calculated creatinine clearance, hepatic function status, known comorbidities, allergies, current medications, and pregnancy status.
...
function computeLocalDosing(patient: any, metrics: any, drug: any) {
  try {
    ...
    console.error("Error in computeLocalDosing:", err);
    return {
      error: "Dosing calculation failed",
      disclaimer: DISCLAIMER,
    };
  }
}
